...
Be specific with basement codes (B1/B2/B3), free slot counts, EV chargers, and INR cost estimates.
...
export function answerFromContext(message, context) {
  ...
  if (looksLike(message, [/where.*(park|should)/i, /which.*(basement|floor|level).*park/i, 'where should i park'])) {
    ...
    return [
      ...
      'Registered company cars allot FCFS into their company pool; unknown plates become guests on B1.',
    ]
      .filter(Boolean)
      .join('\n\n');
  }
  ...
  if (looksLike(message, [/ev/i, /charg/i, /electric/i])) {
    ...
    return [
      ...
      'Most EV bays are on company floors **B2** and **B3**; B1 has a smaller visitor EV set.',
    ].join('\n\n');
  }
  ...
  return [
    ...
    context.recommendations.guestOrVisitor
      ...
      : null,
    'Try asking: where should I park, how crowded is it, which floor has EV charging, or cost for 3 hours.',
  ]
    .filter(Boolean)
    .join('\n');
}
